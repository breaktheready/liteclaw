...
- **Python** 3.10+
- **tmux** 3.0+
- **Claude Code CLI** 설치 및 인증 완료 (`claude --version`으로 확인)
- **텔레그램 봇 토큰** ([@BotFather](https://t.me/BotFather)에서 발급)
- **(권장) OpenAI 호환 API 프록시** — 응답 요약용 (Tier 1)
  - 없으면 Claude Code 에이전트(Tier 2) 또는 원본 출력(Tier 3)으로 자동 대체
  - 추천: [claude-max-api-proxy](https://github.com/mattschwen/claude-max-api-proxy) — Claude Max 구독 재사용, `docker-compose.yml` 제공
  - OpenAI, Groq, 로컬 LLM 등 OpenAI 호환 엔드포인트는 모두 사용 가능 — `SUMMARIZER_URL` / `SUMMARIZER_MODEL`만 맞춰주면 됨
  - 기본 `SUMMARIZER_URL`은 `http://localhost:3456/v1` (추천 프록시의 기본 포트)
- **(선택) Docker** — API 프록시가 컨테이너로 실행되는 경우만 필요
...
- [claude-max-api-proxy](https://github.com/mattschwen/claude-max-api-proxy) — Claude Max 구독을 OpenAI 호환 API로 노출 (권장 Tier 1 요약기)
