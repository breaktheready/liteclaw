...
- **(권장) OpenAI 호환 API 프록시** — 응답 요약용 (Tier 1)
  ...
  - 예시: [claude-max-api-proxy](https://github.com/1rgs/claude-max-openai-proxy) 또는 기타 OpenAI 호환 엔드포인트
  - Docker 사용 시: 프록시 디렉토리에서 `docker compose up -d`
- **(선택) Docker** — API 프록시가 컨테이너로 실행되는 경우만 필요
- **(선택) Node.js** — OAuth 자동 복구(`auto_approve_oauth.js`)에만 필요
