...
저는 전문 개발자가 아니라, 그냥 매일 Claude Code를 쓰는 사람입니다. 쓰던 원격 접속 도구(OpenClaw)가 어느 날 갑자기 안 되면서, 핸드폰에서 Claude Code를 쓸 수가 없게 됐습니다.

그래서 간단하게 하나 만들었습니다. 텔레그램 봇이 tmux에 타이핑하고, 화면에 나온 걸 읽어서 돌려주는 겁니다. 진짜 이게 전부예요. API 호출도 없고, SDK도 없고, 클라우드 서비스도 없습니다 — 그냥 `tmux send-keys`랑 `capture-pane`.

매일 쓰다 보니 꽤 잘 돌아가서, 비슷한 상황에 있는 분들한테 도움이 될까 싶어 공유합니다. Claude Max 구독 중인데 추가 API 비용 없이 원격 접속하고 싶다면 한번 써보세요.
